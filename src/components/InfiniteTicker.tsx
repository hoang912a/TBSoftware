...
export default function InfiniteTicker() {
  ...
  return (
    <section className="relative overflow-hidden border-y border-black/5 bg-[#fafafa] py-4">
      ...
      >
        {tickerItems.map((item, i) => (
          <div
            key={i}
            className="flex items-center gap-3 text-sm font-medium text-apple-gray-text"
          >
            <span className="flex h-2 w-2 rounded-full bg-apple-blue" />
            <span>{item}</span>
          </div>
        ))}
      </motion.div>
